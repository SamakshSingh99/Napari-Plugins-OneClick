...
SCRIPT_DIR="${0:A:h}"
ENV_FILE="$SCRIPT_DIR/plantseg-macos.yaml"
ENV_NAME="plant-seg-dev"
DESKTOP_APP="$HOME/Desktop/Napari PlantSeg.app"
...
if [[ ! -f "$ENV_FILE" ]]; then
    ...
    echo "Keep this installer and plantseg-macos.yaml in the same folder."
    exit 1
fi
...
"$CONDA_EXE" run -n "$ENV_NAME" plantseg --version
"$CONDA_EXE" run -n "$ENV_NAME" python -c \
    "import importlib.metadata as m; import torch; print('PlantSeg:', m.version('plantseg')); print('napari:', m.version('napari')); print('Torch MPS:', torch.backends.mps.is_available())"

echo "Creating the Desktop application..."
if [[ -e "$DESKTOP_APP" ]]; then
    BACKUP_APP="$HOME/.Trash/Napari PlantSeg backup $(date +%Y%m%d-%H%M%S).app"
    mv "$DESKTOP_APP" "$BACKUP_APP"
    echo "The previous Desktop launcher was moved to the Trash as a backup."
fi

LAUNCH_COMMAND="export PYTORCH_ENABLE_MPS_FALLBACK=1; nohup '$CONDA_EXE' run --no-capture-output -n '$ENV_NAME' plantseg -n > /tmp/napari-plantseg.log 2>&1 &"
...
echo "Open “Napari PlantSeg” from your Desktop."
echo "If macOS blocks the first launch, right-click it and choose Open."
echo "Launch log: /tmp/napari-plantseg.log"
echo
read "?Press Return to close this window..."
