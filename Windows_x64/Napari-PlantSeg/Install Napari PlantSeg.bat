...
if not errorlevel 1 (
  "%CONDA_EXE%" run -n "%ENV_NAME%" python -c "import sys, torch; assert sys.version_info[:2] == (3,12); assert torch.__version__.startswith('2.7.1+cpu')" >nul 2>&1
  if errorlevel 1 (
    echo The existing environment has an incompatible or broken PyTorch runtime.
    echo Recreating only the %ENV_NAME% environment with the supported Windows build...
...
echo Checking the native PyTorch runtime...
"%CONDA_EXE%" run -n "%ENV_NAME%" python -c "import torch; assert torch.__version__.startswith('2.7.1+cpu'); print('PyTorch:',torch.__version__); print('CPU runtime: OK')"
if errorlevel 1 (
  echo Repairing PyTorch with the official Windows CPU wheel...
  "%CONDA_EXE%" run -n "%ENV_NAME%" python -m pip install --no-cache-dir --force-reinstall --no-deps "torch==2.7.1+cpu" --index-url https://download.pytorch.org/whl/cpu
  if errorlevel 1 goto :failed
  "%CONDA_EXE%" run -n "%ENV_NAME%" python -c "import torch; assert torch.__version__.startswith('2.7.1+cpu'); print('PyTorch repair: OK')"
  if errorlevel 1 goto :failed
)
...
if /I "%FOLDER_NAME%"=="Napari-PlantSeg" "%CONDA_EXE%" run -n "%ENV_NAME%" python -c "import sys, torch, plantseg; assert sys.version_info[:2] == (3,12); assert torch.__version__.startswith('2.7.1+cpu'); print('Python:',sys.version.split()[0]); print('PyTorch:',torch.__version__); print('PlantSeg: OK')"
