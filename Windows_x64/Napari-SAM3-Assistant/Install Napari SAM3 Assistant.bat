...
set "ENV_FILE=%~dp0napari-sam3-windows.yaml"
set "ENV_NAME=napari-sam3-windows"
...
set "SAM3_COMMIT=477cd99b37f634f8aa06b83936b1de669f339d00"
set "SAM3_SOURCE=%LOCALAPPDATA%\NapariPluginSources\sam3-device-agnostic-477cd99"
if /I "%FOLDER_NAME%"=="Napari-SAM3-Assistant" (
  if not exist "%LOCALAPPDATA%\NapariPluginSources" mkdir "%LOCALAPPDATA%\NapariPluginSources"
  if not exist "%SAM3_SOURCE%\sam3\sam\transformer.py" (
    if exist "%SAM3_SOURCE%" rmdir /s /q "%SAM3_SOURCE%"
    echo Installing the complete pinned SAM3 Python source code...
    echo No SAM3 model weights or checkpoints are downloaded by this installer.
    "%CONDA_EXE%" run -n "%ENV_NAME%" git clone --quiet --branch device-agnostic --single-branch https://github.com/jveitchmichaelis/sam3.git "%SAM3_SOURCE%"
    if errorlevel 1 goto :failed
  )
  "%CONDA_EXE%" run -n "%ENV_NAME%" git -C "%SAM3_SOURCE%" checkout --quiet --detach "%SAM3_COMMIT%"
  if errorlevel 1 goto :failed
  echo Registering the complete SAM3 source packages...
  "%CONDA_EXE%" run -n "%ENV_NAME%" python -c "from pathlib import Path; import site; source=r'''%SAM3_SOURCE%'''; target=Path(site.getsitepackages()[0])/'sam3-complete-source.pth'; target.write_text('import sys; sys.path.insert(0, '+repr(source)+')\n',encoding='utf-8'); print('SAM3 source path:',source)"
  if errorlevel 1 goto :failed
)
...
if /I "%FOLDER_NAME%"=="Napari-SAM3-Assistant" "%CONDA_EXE%" run -n "%ENV_NAME%" python -c "from pathlib import Path; import importlib.metadata as m, einops, pycocotools, napari_sam3_assistant, sam3; import sam3.sam.transformer; assert 'NapariPluginSources' in str(Path(sam3.__file__).resolve()); print('einops:',einops.__version__); print('pycocotools:',m.version('pycocotools')); print('SAM3 source:',Path(sam3.__file__).resolve()); print('sam3.sam: OK'); print('SAM3 Assistant: OK'); print('Model weights: not downloaded')"
...
if /I "%FOLDER_NAME%"=="Napari-SAM3-Assistant" echo No model weights were downloaded. Select your checkpoint path inside napari.
echo.
pause
exit /b 0
...
echo Expected: napari-sam3-windows.yaml
goto :failed
