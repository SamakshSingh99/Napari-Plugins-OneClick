...
set "ENV_FILE=%~dp0napari-n2v-windows.yaml"
set "ENV_NAME=napari-n2v"
...
if /I "%FOLDER_NAME%"=="Napari-Noise2Void" (
  echo Checking the native Windows TensorFlow and Keras runtime...
  "%CONDA_EXE%" run -n "%ENV_NAME%" python -c "import tensorflow as tf; import tensorflow.keras as tk; assert tf.__version__=='2.15.1'; print('TensorFlow:',tf.__version__); print('tensorflow.keras: OK')" >nul 2>&1
  if errorlevel 1 (
    echo The TensorFlow runtime is incomplete; reinstalling its Windows wheels...
    "%CONDA_EXE%" run -n "%ENV_NAME%" python -m pip install --force-reinstall --no-deps "tensorflow==2.15.1" "tensorflow-intel==2.15.1" "keras==2.15.0"
    if errorlevel 1 goto :failed
  )
  "%CONDA_EXE%" run -n "%ENV_NAME%" python -c "import tensorflow as tf; import tensorflow.keras as tk; assert tf.__version__=='2.15.1'; print('TensorFlow:',tf.__version__); print('tensorflow.keras: OK')"
  if errorlevel 1 goto :failed
)
...
if /I "%FOLDER_NAME%"=="Napari-Noise2Void" "%CONDA_EXE%" run -n "%ENV_NAME%" python -c "import tensorflow as tf, n2v, napari_n2v; print('TensorFlow:',tf.__version__); print('Noise2Void plugin: OK')"
...
echo Expected: napari-n2v-windows.yaml
goto :failed
