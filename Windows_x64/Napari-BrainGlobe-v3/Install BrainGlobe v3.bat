...
set "CONDA_PKGS_DIRS=%LOCALAPPDATA%\NapariPluginInstallCache\BrainGlobe-v3-rev5"
if not exist "%CONDA_PKGS_DIRS%" mkdir "%CONDA_PKGS_DIRS%"
echo Using a fresh BrainGlobe package cache: %CONDA_PKGS_DIRS%
set "ENV_EXISTS=0"
"%CONDA_EXE%" env list | findstr /r /c:"^%ENV_NAME%[ ]" >nul
if not errorlevel 1 (
  set "ENV_EXISTS=1"
  echo Checking the existing Python runtime...
  "%CONDA_EXE%" run -n "%ENV_NAME%" python -c "import ctypes, ssl, sqlite3; print('Existing Python runtime: OK')" >nul 2>&1
  if errorlevel 1 (
    echo.
    echo The existing BrainGlobe environment is incomplete or corrupted.
    echo Rebuilding only %ENV_NAME% to restore missing Python DLLs...
...
if "%ENV_EXISTS%"=="1" (
  echo The environment already exists; restoring the core runtime...
  "%CONDA_EXE%" install --name "%ENV_NAME%" --yes --override-channels --channel conda-forge --strict-channel-priority python=3.12.11 pip napari=0.6.6 pyqt6 libffi vedo vtk
) else (
  "%CONDA_EXE%" create --name "%ENV_NAME%" --yes --override-channels --channel conda-forge --strict-channel-priority python=3.12.11 pip napari=0.6.6 pyqt6 libffi vedo vtk
)
if errorlevel 1 goto :failed

echo Checking the Python runtime before installing the BrainGlobe suite...
"%CONDA_EXE%" run -n "%ENV_NAME%" python -c "import ctypes, ssl, sqlite3; print('Python runtime and DLLs: OK')"
if errorlevel 1 (
  echo The Python runtime check failed after installation.
  echo Security software may have quarantined a DLL from the Conda environment.
  goto :failed
)

echo Installing the pinned BrainGlobe suite...
"%CONDA_EXE%" run -n "%ENV_NAME%" python -m pip install --upgrade --upgrade-strategy only-if-needed "brainglobe==3.0.0" "brainglobe-atlasapi==3.0.1" "brainglobe-heatmap==0.6.0" "brainglobe-napari-io==0.5.0" "brainglobe-segmentation==1.3.3" "brainglobe-space==1.0.3" "brainglobe-utils==0.11.2" "brainreg[napari]==1.0.15" "brainrender-napari==0.2.0" "brainrender==2.2.1" "cellfinder[napari]==1.10.1"
if errorlevel 1 goto :failed
