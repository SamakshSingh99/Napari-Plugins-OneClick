...
SCRIPT_DIR="${0:A:h}"
ENV_FILE="$SCRIPT_DIR/napari-sam3-macos.yaml"
ENV_NAME="napari-sam3"
DESKTOP_APP="$HOME/Desktop/Napari SAM3.app"
...
if [[ ! -f "$ENV_FILE" ]]; then
    ...
    echo "Keep this installer and napari-sam3-macos.yaml in the same folder."
    exit 1
fi
...
"$CONDA_EXE" run -n "$ENV_NAME" python -c \
    "import napari, numpy, scipy, torch, sam3; import sam3.sam.transformer; print('Napari SAM3 imports: OK')"

echo "Creating the Desktop application..."
if [[ -e "$DESKTOP_APP" ]]; then
    BACKUP_APP="$HOME/.Trash/Napari SAM3 backup $(date +%Y%m%d-%H%M%S).app"
    mv "$DESKTOP_APP" "$BACKUP_APP"
    echo "The previous Desktop launcher was moved to the Trash as a backup."
fi

LAUNCH_COMMAND="export PYTORCH_ENABLE_MPS_FALLBACK=1; nohup '$CONDA_EXE' run --no-capture-output -n '$ENV_NAME' napari > /tmp/napari-sam3.log 2>&1 &"
...
echo "Open “Napari SAM3” from your Desktop."
echo "If macOS blocks the first launch, right-click it and choose Open."
echo "Launch log: /tmp/napari-sam3.log"
echo
read "?Press Return to close this window..."
